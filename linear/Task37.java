...
public class Task37 {
	...
	sumOfDigits = someDigitNumber / 100
	...
	/*
	 * e) Квадрат заданного трехзначного числа 
	...
	cubeOfDigitsSum = Math.pow(digitOne, 3)
		          + Math.pow(digitTwo, 3)
		          + Math.pow(digitThree, 3)
		          + 3 * Math.pow(digitOne, 2) * digitTwo
	...
	isIsoscelesTriangle = ((sideA == sideB)
			       || (sideB == sideC)
			       || (sideA == sideC));
	...
	/*
	 * i) График функции y = ax^2 + bx + c 
	 *    проходит через заданную точку с координатами (m, n).
	 */
